...
class MastermindGame:
    def __init__(self):
        self.code_length = 4
        self.possible_nums = 6 #0 to 5 (possible_nums-1)
        self.code = (0,)
...
##----------------------------------------------------------------------------##
    ...
    def count_digits(self, code):
        ...
        for num in range(0, self.possible_nums):
            digit_count[num] = 0
...
##----------------------------------------------------------------------------##
    ...
    def check_guess(self, guess):
        ...
        count = self.count_digits(code)
        guesser = self.count_digits(guess)
        for item in guesser:
                if guesser[item] <= count[item] != 0:
                    correct_nums += guesser[item]
                elif guesser[item] > count[item] != 0:
                    correct_nums += count[item]
